...
class Tcp(Protocol):
    ...
    def __init__(self, ip, number):
        message_data = ip.packet[ip.header_len:]
        ...
        self.segement_len = 0  # HUA = ip.total - ip.header_len - self.header_len
    ...
    def _decode(self):
        ...
        self.header_len = int(data_to_hex_str(self.message[12])[0:3], 16) * 4
        self.segement_len = self.ip.total_len - self.ip.header_len - self.header_len
        ...
        if (self.header_len > 20):
            self.opt_paddings = data_to_hex_str(self.message[20 : self.header_len])
    #endof def
